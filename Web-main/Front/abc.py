...
LLM_ENV_PATH = ROOT_DIR / "NLP-jm" / "LLM" / ".env"
GLOBAL_ENV_PATH = ROOT_DIR / "NLP-jm" / ".env"
...
<body>
  ...
  <script>
    ...
    const DEFAULT_INGREDIENTS = ["소금", "설탕", "간장", "식용유", "마늘", "파", "후춧가루"];
    const state = {
      mode: "camera",
      stream: null,
      ingredients: [],
    ...
    function bindElements() {
      [
        ...
        "settingsButton", "basicIngredients"
    ...
    function wireMainEvents() {
      ...
      els.askButton.addEventListener("click", async () => {
        try { await askChef(); } catch (error) { setStatus(error.message, true); addMessage("system", error.message); }
      });
      ...
      els.askText.addEventListener("input", scheduleVideoTracking);
    ...
    function addSelectedBasicIngredients() {
      const selected = [...els.basicIngredients.querySelectorAll("input:checked")]
    ...
    function addMessage(role, text, video = null) {
      if (!text) return;
      state.messages.push({ role, text, video });
      renderChat();
    }

    function renderChat() {
      els.chatLog.innerHTML = state.messages.length
        ? state.messages.map((msg) => `<div class="msg ${msg.role}">${escapeHtml(msg.text)}${renderVideo(msg.video)}</div>`).join("")
        : `<div class="msg system">대화가 여기에 표시됩니다.</div>`;
      els.chatLog.scrollTop = els.chatLog.scrollHeight;
    }

    async function confirmIngredients() {
      if (!state.ingredients.length) throw new Error("확정할 재료가 없습니다.");
    ...
    async function askChef(text = els.askText.value.trim()) {
      ...
      try {
        ...
        addMessage("assistant", answer, video);
        if (data.youtube_status?.requested && data.youtube_status?.message) {
    ...
    async function speakText(text) {
      ...
      await new Promise((resolve) => {
        ...
        const finish = () => {
          ...
          if (state.speechSeq === speechSeq) setSpeaking(false);
          resolve();
        };
    ...
    async function handleVoiceCommand(text) {
      ...
      try {
        ...
        if (state.ingredients.length && isConfirmText(text)) {
          ...
          if (!data?.cancelled) await speakText(data?.answer || "");
        }
      } catch (error) {
        setStatus(error.message, true);
